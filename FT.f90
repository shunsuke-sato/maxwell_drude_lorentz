program main
  ...
  integer,parameter :: nt = 33073, nw = 100
  ...
  tmid = sum(tt*Et_vac_wo_np**2)/sum(Et_vac_wo_np**2)
  ...
  do iw = 0, nw
    ...
    zt1 = 0d0; zt2 = 0d0
    do it = 0,nt
      zt1 = zt1 + exp(zi*ww*(tt(it)-tmid))*Et_vac_w_np(it)
      zt2 = zt2 + exp(zi*ww*(tt(it)-tmid))*Et_vac_wo_np(it)
    end do
  ...
  open(30,file="phase.out")
  do iw = 0, nw
    ww = wi + dw*iw
    write(30,"(999e26.16e3)")ww,aimag(log(zEw_vac_wo_np(iw))),aimag(log(zEw_vac_w_np(iw)))
  end do
  close(30)
